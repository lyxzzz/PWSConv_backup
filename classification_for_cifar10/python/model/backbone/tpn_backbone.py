...
def backbone(inputs, backbone_cfg, network_cfg, is_training=True, scope='tpn_backbone'):
    ...
    dense_block = cfg_utils.get_cfg_attr(backbone_cfg, 'dense_block', 4)
    ...
    with slim.arg_scope([norm_func], is_training=is_training):
        with tf.variable_scope(scope, 'tpn_backbone'):
            ...
            net = dense_module(net, growth_num, bottleneck_ratio, dense_block, 'denseblock3', rate=2)
            net = transition_module(net, transition_ratio, 'transitblock3', pool=False)
            tmp_points['block3_2'] = net

            depth = 256

            net = tf.concat([tmp_points['block3_0'], tmp_points['block3_1'], tmp_points['block3_2']], axis=-1)
            net = slim.conv2d(net, depth, [1, 1], stride=1, scope='conv1x1_0')
            ...
            with tf.variable_scope('block_5'):
                ...
                start = slim.conv2d(net, output_depth, [3, 3], stride=1, scope='conv2')#[75,75]
                net = slim.conv2d(start, output_depth, [3, 3], rate=2, scope='conv3')#[75,75]
            ...
            with tf.variable_scope(end_point):
                ...
                net = slim.conv2d(net, output_depth, [3, 3], stride=2, scope='conv3x3')
            ...
            with tf.variable_scope(end_point):
                ...
                net = slim.conv2d(net, output_depth/2, [3, 3], stride=2, scope='conv3x3')
            end_points[end_point] = [net]
            ...
            #     net = slim.conv2d(net, 512, [3, 3], stride=1, scope='conv1')
