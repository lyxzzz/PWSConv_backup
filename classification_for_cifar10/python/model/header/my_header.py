...
def arg_scope(network_cfg):
    ...
    with slim.arg_scope([tf_layer.unbalance_conv, slim.conv2d],
        ...
        # variance_scaling_initializer(factor=2.0, mode='FAN_AVG', uniform=True)
...
def _locsubnet(net, output_nums, subnet_nums, subnet_channels, norm_func, last_layer_norm_func, num_anchors):
    ...
    for i in range(subnet_nums):
        net = tf_layer.unbalance_conv(net, subnet_channels, 3, stride=1, padding="SAME",
                                  scope='loc_{}'.format(i))
    outputs = tf_layer.unbalance_conv(net, output_nums, 3,
                               activation_fn=None,
                               normalizer_fn=None,
                               padding="SAME",
                               scope="loc_pred")
