...
def wn_conv(inputs, channels, kernel_size, weights_initializer, weights_regularizer, stride=1,
    ...
    with tf.variable_scope(scope, 'WNConv', [inputs], reuse=reuse) as sc: 
        ...
        n = kernel_size[0] * kernel_size[1] * C 

        def __cal_shift(k, g, n):
            m, v = tf.nn.moments(k, [0, 1, 2], name='moments')
            stddev = tf.sqrt(v)
            _, shift_k = tf.nn.moments(n * m, [0])

            m1, v1 = tf.nn.moments(g, [0, 1, 2], name='moments')
            stddev1 = tf.sqrt(v1)
            m1 = m1 / stddev1 * stddev
            _, shift_g = tf.nn.moments(n * m1, [0])

            return shift_k, shift_g

        V = tf.nn.l2_normalize(kernel, [0, 1, 2])

        raw, process = __cal_shift(kernel, V, n)
        tf.add_to_collection("raw", raw)
        tf.add_to_collection("process", process)
...
def _initializer(shape, dtype, partition_info=None, seed=None):
...
def pws_conv(inputs, channels, kernel_size, weights_initializer, weights_regularizer, stride=1,
    ...
    with tf.variable_scope(scope, 'PWSConv', [inputs], reuse=reuse) as sc: 
        ...
        if single_beta == True:
            ...
            bias_shape = [channels]
        in_var_value = kernel_size[0] * kernel_size[1] * C 
        out_var_value = kernel_size[0] * kernel_size[1] * channels
        var_value = math.sqrt(2 / float(in_var_value))
        ...
        multi_gamma = slim.variable("gamma", shape=bias_shape,
                initializer=tf.constant_initializer(1.0),
                # device=None,
                trainable=True)

        multi_gamma = tf.reshape(multi_gamma, [1, 1, 1, -1])
    
        mean, var = tf.nn.moments(root_conv, [0, 1, 2], name='moments', keep_dims=True)
        mean = tf.reshape(mean, [1, 1, 1, -1])
        var = tf.reshape(var, [1, 1, 1, -1])
        root_conv = multi_gamma * var_value * tf.nn.batch_normalization(root_conv, mean, var, None, None, epsilon)
...
def myconv(inputs, channels, kernel_size, weights_initializer, weights_regularizer, stride=1, epsilon=0.001, biases_initializer=0, single_beta=True,
    ...
    with tf.variable_scope(scope, 'MyConv', [inputs], reuse=reuse) as sc:
        ...
        tf.add_to_collection("kernel_real_var", tf.reduce_min(var))
...
# def image_subtraction(images, means=[125., 122., 113.], var=[51., 52., 51.]):
#     result = images - means
#     result = result / var
#     return result
