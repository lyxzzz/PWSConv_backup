...
class EpochRecorder:
    def __init__(self, loss_dict, summary_writer, restore_epoch, max_epoch, isMAP=True):
        self.loss_dict = loss_dict
        self.program_start = time.time()
        self.epoch_start = self.program_start
        self.acc_dict = ['f-measure', 'precision', 'recall']
    ...
    def summary_epoch(self, loss_list, acc_list, learning_rate, epochs, steps, steps_per_epoch, scope='train'):
        ...
        if self.isMAP:
            ...
            acc_val = eval_tools.f_measure(acc_list)
        ...
        if self.isMAP:
            tag_name = acc_name + "map"
            summary_epoch.value.add(tag=tag_name, simple_value=acc_val)
        else:
        ...
        print('\n\tacc condition',end='')
        if self.isMAP:
            print(", map[{:.2f} ~ {:.2f}]".format(last_acc, acc_val), end='')
        else:
            for index,name in enumerate(self.acc_dict):
